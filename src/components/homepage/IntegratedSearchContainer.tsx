...
const IntegratedSearchContainer: React.FC<IntegratedSearchContainerProps> = ({
  ...
  return (
    <div ref={containerRef} className="w-full">
      {/* Clean Search Form - No Containers */}
      <form onSubmit={handleSearch}>
        ...
        <div className="flex gap-3">
          ...
          {/* Upload File Button - Matching Navigation Bar Style */}
          <div className="relative group">
            <Tooltip
              content={
                <div className="px-1 py-2">
                  <div className="text-sm">Upload audio files (MP3, WAV, FLAC), any length, no limit.</div>
                </div>
              }
            >
              <Link
        ...
        <div className="mt-4 bg-white/95 dark:bg-content-bg/95 backdrop-blur-sm rounded-2xl p-4 shadow-lg border border-gray-200/50 dark:border-gray-600/50">
          {isSearching ? (
            <div className="flex flex-col items-center py-6">
              ...
              <p className="text-center text-gray-800 dark:text-gray-200 font-medium mt-2">Searching YouTube...</p>
              <p className="text-center text-gray-600 dark:text-gray-300 text-sm mt-1">Results will appear in a moment</p>
            </div>
          ) : (
            <>
              ...
              <div className="max-h-60 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-400 dark:scrollbar-thumb-gray-500 scrollbar-track-transparent">
                <div className="space-y-3 pr-2">
                  {displayedResults.map((result) => (
                    <div
                      key={result.id}
                      className="flex cursor-pointer bg-gray-50/80 dark:bg-content-bg/80 hover:bg-white dark:hover:bg-content-bg p-3 rounded-lg transition-all duration-200 shadow-sm hover:shadow-md border border-gray-200/60 dark:border-gray-600/60 hover:border-gray-300 dark:hover:border-gray-500"
