...
def validate_file_size(file: FileStorage, detector: str, force: bool) -> Tuple[bool, Optional[str]]:
    ...
    # Size limits based on detector - Set to high val to allow any size
    size_limits = {
        'beat-transformer': 15000,
        'madmom': 15000,
        'librosa': 15000
    }
    ...
    if not force:
        ...
        elif detector == 'auto' and file_size_mb > 15000:
            # For auto, suggest alternatives for large files
            return False, f"File too large ({file_size_mb:.1f}MB > 15000MB). Specify detector='madmom' or 'librosa' for larger files, or add 'force=true'."
